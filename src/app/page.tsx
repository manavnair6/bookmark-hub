...
export default function Home() {
  ...
  // Uses 'uid' to match your Supabase table schema
  ...
  // Matches your Supabase columns: link_title, link_url, uid
  ...
  if (!u) return (
    <div className="min-h-screen bg-[#0f172a] flex items-center justify-center p-4">
      <div className="bg-[#1e293b] p-10 rounded-2xl shadow-2xl border border-slate-700 text-center max-w-md w-full">
        ...
        <button onClick={login} className="w-full bg-white hover:bg-slate-100 text-slate-900 font-bold py-3 px-6 rounded-xl transition-all flex items-center justify-center gap-3 cursor-pointer">
          Sign in with Google
        </button>
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <header className="flex justify-between items-center mb-12">
          ...
          <button onClick={logout} className="text-sm font-medium text-slate-400 hover:text-white transition-colors cursor-pointer">Sign Out</button>
        </header>
        ...
        <form onSubmit={addBm} className="bg-[#1e293b] p-6 rounded-2xl border border-slate-700 shadow-xl mb-10">
          ...
          {/* Added cursor-pointer */}
          <button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold py-3 rounded-lg flex items-center justify-center gap-2 transition-all cursor-pointer">
          ...
          {bms.map((b) => (
            <div key={b.id} className="bg-[#1e293b] border border-slate-700 p-5 rounded-xl flex justify-between items-center group hover:border-blue-500 transition-all">
              ...
              <div className="flex gap-2">
                <a href={b.link_url} target="_blank" className="p-2 hover:bg-slate-700 rounded-lg text-blue-400 cursor-pointer">
                  <ExternalLink className="w-5 h-5" />
                </a>
                {/* Added cursor-pointer */}
                <button onClick={() => delBm(b.id)} className="p-2 hover:bg-red-500/10 rounded-lg text-slate-500 hover:text-red-400 cursor-pointer">
